...
df1 = pd.read_csv('data.csv').replace('#VALUE!', np.NaN)
df2 = df1.dropna().drop('indx_members', axis=1)

X = df2.drop('cur_mkt_cap', axis=1)
y = df2['cur_mkt_cap'].values
...
print(predictions)
for i in range(len(index_values)):
    ticker = df1.loc[index_values[i]]['indx_members']
    actual = df2.loc[index_values[i]]['cur_mkt_cap']
    predicted = predictions[i]
    print(ticker, actual, predicted, predicted/actual)
df = df2.astype(float)
corrM = df.corr()
